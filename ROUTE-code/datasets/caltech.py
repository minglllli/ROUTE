...
def p_u_split_two_sample(cfg, labels):
    ...
    positive_idx = []
    unlabeled_idx = []
    num_p_u = int(cfg.num_u*cfg.p_prior)
    num_n_u = cfg.num_u - num_p_u
    #pos_per_class = cfg.num_p // len(cfg.positive_label_list)
    #unlabel_per_class = cfg.num_u // 100 #eurosat
    ...
    positive_idx = np.random.choice(positive_idx, cfg.num_p + num_p_u, False)
    positive_idx_label = positive_idx[:cfg.num_p]
    positive_idx_unlabeled = positive_idx[cfg.num_p:]

    unlabeled_idx = np.random.choice(unlabeled_idx, num_n_u, False)
    unlabeled_idx = unlabeled_idx.extend(positive_idx_unlabeled)
    positive_idx = np.array(positive_idx_label)
